...
from ai.result_interpreter.providers import ProviderSettings, create_explanation_provider
...
class IntegratedModelApp(ExplanationPanelMixin):
    def __init__(
        ...
        initial_provider = create_explanation_provider(ProviderSettings.from_environment("auto"))
        self.explanation_service = ExplanationService(initial_provider)
        initial_provider_choice = "external_llm" if initial_provider.name == "external_llm" else "mock"
        ...
        mode_box = ttk.Combobox(provider_controls, textvariable=self.explanation_mode_var,
                                values=("Analyzer-guided", "Evidence-assisted", "Extracted-only"), state="readonly", width=18)
        mode_box.pack(side=tk.LEFT); mode_box.bind("<<ComboboxSelected>>", self._explanation_mode_changed)
