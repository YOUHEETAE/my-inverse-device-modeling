...
from ai.visualization.field_data import (
    FIELD_DISPLAYS,
    RANGE_MODES,
    SCALE_MODES,
    GeneratedFieldMap,
)
from ai.visualization.field_rendering import (
    render_model_field,
    render_model_field_comparison,
)
from ai.result_interpreter import ExplanationService, MockExplanationProvider
from ai.result_interpreter.providers import (
    ProviderSettings,
    create_explanation_provider,
)
from ai.visualization.curve_rendering import render_curve_figure
from ai.visualization.explanation_panel import ExplanationPanelMixin
from ai.visualization.guide_panel import build_guide_panel
from ai.visualization.guide_content import GUIDE_CHAPTERS, THEORY_CHAPTERS
...
class IntegratedModelApp(ExplanationPanelMixin):
    def __init__(
        ...
        self.geo_template = geo_template
        ...
        initial_provider = create_explanation_provider(
            ProviderSettings.from_environment("auto")
        )
        self.explanation_service = ExplanationService(initial_provider)
        initial_provider_choice = (
            "external_llm" if initial_provider.name == "external_llm" else "mock"
        )
        self.explanation_provider_var = tk.StringVar(value=initial_provider_choice)
        self.explanation_mode_var = tk.StringVar(value="Analyzer-guided")
        ...
        self.explanation_history: dict[str, dict[tuple, tuple[str, bool]]] = {
            "curve": {},
            "field": {},
        }
        ...
        top = ttk.Frame(window, padding=(10, 8))
        top.pack(side=tk.TOP, fill=tk.X)
        provider_row = ttk.Frame(window, padding=(10, 0, 10, 8))
        provider_row.pack(side=tk.TOP, fill=tk.X)
        provider_controls = ttk.Frame(provider_row)
        provider_controls.pack(side=tk.RIGHT)
        ttk.Label(provider_controls, text="Explanation Provider").pack(
            side=tk.LEFT, padx=(0, 3)
        )
        provider_box = ttk.Combobox(
            provider_controls,
            textvariable=self.explanation_provider_var,
            values=("mock", "external_llm", "auto"),
            state="readonly",
            width=12,
        )
        provider_box.pack(side=tk.LEFT)
        provider_box.bind("<<ComboboxSelected>>", self._provider_changed)
        ttk.Label(provider_controls, text="Mode").pack(side=tk.LEFT, padx=(10, 3))
        mode_box = ttk.Combobox(
            provider_controls,
            textvariable=self.explanation_mode_var,
            values=("Analyzer-guided", "Evidence-assisted", "Extracted-only"),
            state="readonly",
            width=18,
        )
        mode_box.pack(side=tk.LEFT)
        mode_box.bind("<<ComboboxSelected>>", self._explanation_mode_changed)

        ttk.Label(
            top, text="Common device parameters", font=("TkDefaultFont", 10, "bold")
        ).pack(side=tk.LEFT, padx=(0, 10))
        self.parameter_vars = {
            name: tk.StringVar(value=DEFAULT_PARAMETERS[name]) for name in PARAMETERS
        }
        for name in PARAMETERS:
            ttk.Label(top, text=PARAMETER_TITLES[name]).pack(side=tk.LEFT)
            combo = ttk.Combobox(
                top,
                textvariable=self.parameter_vars[name],
                values=PARAMETER_OPTIONS[name],
                state="normal",
                width=11,
            )
            combo.pack(side=tk.LEFT, padx=(3, 8))
            combo.bind("<<ComboboxSelected>>", lambda _event: self._dirty())
            combo.bind("<KeyRelease>", lambda _event: self._dirty())
            combo.bind("<Return>", self._parameters_enter)
        ...
        style.configure(
            "Main.TNotebook.Tab", padding=(18, 8), font=("TkDefaultFont", 10, "bold")
        )
        style.map(
            "Main.TNotebook.Tab",
            background=[("selected", "#d7e8ff"), ("active", "#edf5ff")],
        )
        self.notebook = ttk.Notebook(window, style="Main.TNotebook")
        self.notebook.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0, 8))
        self.curve_tab = ttk.Frame(self.notebook)
        ...
        self.notebook.add(self.curve_tab, text="I–V Curve")
        self.notebook.add(self.field_tab, text="Structure / Field Map")
    ...
    def _provider_changed(self, _event=None) -> None:
        ...
        for kind, status in self.explanation_status.items():
            status.set(f"Ready ({provider.name})")

    def _explanation_mode_changed(self, _event=None) -> None:
        self.explanation_history = {"curve": {}, "field": {}}
        for kind, status in self.explanation_status.items():
            status.set(f"Ready ({self.explanation_mode_var.get()})")

    def _build_curve_tab(self) -> None:
        panel = ttk.LabelFrame(
            self.curve_tab,
            text="I–V curve settings",
            padding=(10, 8),
            width=self.side_panel_width,
        )
        panel.pack(side=tk.RIGHT, fill=tk.Y, padx=(5, 8), pady=4)
        panel.pack_propagate(False)
        tree_frame = ttk.Frame(panel)
        tree_frame.pack(fill=tk.X)
        self.curve_tree = ttk.Treeview(
            tree_frame,
            columns=("curve", *PARAMETERS),
            show="headings",
            height=3,
            selectmode="browse",
        )
        ...
        for name in ("curve", *PARAMETERS):
            self.curve_tree.heading(name, text="Curve" if name == "curve" else name)
            self.curve_tree.column(
                name, width=widths[name], anchor=tk.CENTER, stretch=False
            )
        self.curve_scrollbar = ttk.Scrollbar(
            tree_frame, orient=tk.VERTICAL, command=self.curve_tree.yview
        )
        ...
        buttons.pack(fill=tk.X, pady=(6, 3))
        ttk.Button(buttons, text="Add Curve", command=self.add_curve).pack(
            side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 2)
        )
        ttk.Button(
            buttons, text="Update Selected", command=self.update_selected_curve
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        ttk.Button(
            buttons, text="Remove Selected", command=self.remove_selected_curve
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(2, 0))
        self.curve_view_button = ttk.Button(
            panel, text="Separate Biases (8 plots)", command=self.toggle_curve_view
        )
        self.curve_view_button.pack(fill=tk.X, pady=(2, 8))
        ttk.Separator(panel).pack(fill=tk.X, pady=(0, 7))
        ttk.Label(
            panel,
            text="Extracted electrical parameters",
            font=("TkDefaultFont", 9, "bold"),
        ).pack(anchor="w", pady=(0, 4))
        electrical_container = ttk.Frame(panel)
        electrical_container.pack(fill=tk.X)
        self.electrical_canvas = tk.Canvas(
            electrical_container, height=315, highlightthickness=0
        )
        self.electrical_hscroll = ttk.Scrollbar(
            electrical_container,
            orient=tk.HORIZONTAL,
            command=self.electrical_canvas.xview,
        )
        ...
        self.electrical_canvas_window = self.electrical_canvas.create_window(
            (0, 0), window=self.electrical_panel, anchor="nw"
        )
        self.electrical_panel.bind(
            "<Configure>",
            lambda _event: self.electrical_canvas.configure(
                scrollregion=self.electrical_canvas.bbox("all")
            ),
        )
        self.electrical_value_widgets: list[tk.Widget] = []
        ttk.Label(self.electrical_panel, text="Parameter").grid(
            row=0, column=0, padx=5, pady=(0, 7), sticky="w"
        )
        for row, (name, label, unit, _factor, _fmt) in enumerate(
            ELECTRICAL_PARAMETERS, start=1
        ):
            text = f"{label} ({unit})" if unit else label
            ttk.Label(self.electrical_panel, text=text, justify=tk.LEFT).grid(
                row=row, column=0, padx=5, pady=2, sticky="w"
            )
        ttk.Separator(panel).pack(fill=tk.X, pady=(7, 7))
        explanation = ttk.LabelFrame(panel, text="Explanation", padding=(8, 8))
        explanation.pack(fill=tk.BOTH, expand=True)
        self._build_explanation_panel(explanation, "curve")
        plot_frame = ttk.Frame(self.curve_tab)
        plot_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.curve_figure = Figure(figsize=(8.5, 6), dpi=100)
        self.curve_canvas = FigureCanvasTkAgg(self.curve_figure, master=plot_frame)
        self.curve_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        toolbar = ttk.Frame(plot_frame)
        toolbar.pack(side=tk.BOTTOM, fill=tk.X)
        NavigationToolbar2Tk(self.curve_canvas, toolbar)
    ...
    def _build_field_tab(self) -> None:
        ...
        for label, variable, values, width in (
            ("Field map", self.field_var, FIELD_DISPLAYS, 30),
            ("Scale", self.scale_var, SCALE_MODES, 14),
            ("Range", self.range_var, RANGE_MODES, 18),
        ):
            row = ttk.Frame(panel)
            row.pack(fill=tk.X, pady=2)
            ttk.Label(row, text=label, width=10, anchor="w").pack(side=tk.LEFT)
            combo = ttk.Combobox(
                row, textvariable=variable, values=values, state="readonly", width=width
            )
        ...
        ttk.Label(panel, text="Fixed bias: Vg=3 V, Vd=3 V").pack(
            anchor="w", pady=(0, 7)
        )
        selection_header = ttk.Frame(panel)
        selection_header.pack(fill=tk.X, pady=(0, 8))
        ttk.Label(
            selection_header,
            text="Compare curves (maximum 2)",
            font=("TkDefaultFont", 9, "bold"),
        ).pack(side=tk.LEFT)
        ttk.Button(
            selection_header, text="Generate All", command=self.generate_all
        ).pack(side=tk.RIGHT, padx=(6, 0))
        self.field_curve_frame = ttk.Frame(panel)
        self.field_curve_frame.pack(fill=tk.X, pady=(0, 8))
        ttk.Label(panel, text="Parameters", font=("TkDefaultFont", 9, "bold")).pack(
            anchor="w"
        )
        self.field_parameter_frame = ttk.Frame(panel)
        self.field_parameter_frame.pack(fill=tk.X, pady=(3, 8))
        ttk.Separator(panel).pack(fill=tk.X, pady=(4, 8))
        explanation = ttk.LabelFrame(panel, text="Explanation", padding=(8, 8))
        explanation.pack(fill=tk.BOTH, expand=True)
        self._build_explanation_panel(explanation, "field")
        plot_frame = ttk.Frame(self.field_tab)
        plot_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.field_figure = Figure(figsize=(8.5, 6), dpi=100)
        self.field_canvas = FigureCanvasTkAgg(self.field_figure, master=plot_frame)
        self.field_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        toolbar = ttk.Frame(plot_frame)
        toolbar.pack(side=tk.BOTTOM, fill=tk.X)
        NavigationToolbar2Tk(self.field_canvas, toolbar)
    ...
    def _refresh_curve_tree(self) -> None:
        ...
        for index, config in enumerate(self.curve_configs):
            ...
            self.curve_tree.insert(
                ...
                iid=str(index),
                values=(
                    f"{checked} Curve {index + 1}",
                    *(config[name] for name in PARAMETERS),
                ),
            )
        active = min(self.active_curve_index, len(self.curve_configs) - 1)
        self.active_curve_index = active
        self.curve_tree.selection_set(str(active))
        self.curve_tree.focus(str(active))
        if len(self.curve_configs) > 3:
            self.curve_scrollbar.grid()
            self.more_curves_label.grid()
            self.curve_tree.see(str(active))
        else:
            self.curve_scrollbar.grid_remove()
            self.more_curves_label.grid_remove()
    ...
    def _refresh_field_curve_controls(self) -> None:
        ...
        ttk.Label(self.field_parameter_frame, text="Curve").grid(
            row=0, column=0, padx=(0, 7), pady=2, sticky="w"
        )
        for column, name in enumerate(PARAMETERS, start=1):
            ttk.Label(self.field_parameter_frame, text=name).grid(
                row=0, column=column, padx=4, pady=2
            )
        for row, index in enumerate(selected, start=1):
            ttk.Label(self.field_parameter_frame, text=f"Curve {index + 1}").grid(
                row=row, column=0, padx=(0, 7), pady=2, sticky="w"
            )
            for column, name in enumerate(PARAMETERS, start=1):
                ttk.Label(
                    self.field_parameter_frame,
                    text=self.curve_configs[index][name],
                    anchor="e",
                ).grid(row=row, column=column, padx=4, pady=2, sticky="e")
    ...
    def _field_curve_toggled(self, index: int) -> None:
        ...
        self.status.set(
            "Field comparison selection changed. Press Generate All to generate the selected map(s)."
        )

    def _curve_tree_clicked(self, event: tk.Event) -> None:
        if (
            self.curve_tree.identify_region(event.x, event.y) != "cell"
            or self.curve_tree.identify_column(event.x) != "#1"
        ):
            return
        ...
        self._refresh_curve_tree()
        self.render_curves()
        self._restore_explanation("curve")

    def _curve_selected(self, _event=None) -> None:
        selection = self.curve_tree.selection()
        if not selection:
            return
        self.active_curve_index = int(selection[0])
        config = self.curve_configs[self.active_curve_index]
        for name in PARAMETERS:
            self.parameter_vars[name].set(config[name])
        self._update_electrical_panel()
        self.status.set(
            f"Curve {self.active_curve_index + 1} selected. Press Update Selected after editing parameters."
        )

    def add_curve(self) -> None:
        self.curve_configs.append(dict(self._values()))
        ...
        try:
            ...
            messagebox.showerror(
                "Curve generation failed", str(exc), parent=self.window
            )
    ...
    def update_selected_curve(self) -> None:
        ...
        try:
            ...
            self._restore_explanation("curve")
            self.status.set(
                f"Curve {self.active_curve_index + 1} updated and generated."
            )
        except Exception as exc:
            messagebox.showerror(
                "Curve generation failed", str(exc), parent=self.window
    ...
    def remove_selected_curve(self) -> None:
        if len(self.curve_configs) == 1:
            self.status.set("At least one curve must remain.")
            return
        removed = self.active_curve_index
        self.curve_configs.pop(removed)
        self.visible_curve_indices = {
            index - 1 if index > removed else index
            for index in self.visible_curve_indices
            if index != removed
        }
        self.field_selected_indices = {
            index - 1 if index > removed else index
            for index in self.field_selected_indices
            if index != removed
        }
        self.field_outputs = {
            index - 1 if index > removed else index: output
            for index, output in self.field_outputs.items()
            if index != removed
        }
        ...
        self._refresh_curve_tree()
        self._curve_selected()
        self.render_curves()
        self._restore_explanation("curve")
        self._restore_explanation("field")
        self.status.set("Selected curve removed.")

    def toggle_curve_view(self) -> None:
        self.combined_curves = not self.combined_curves
        self.curve_view_button.configure(
            text=(
                "Separate Biases (8 plots)"
                if self.combined_curves
    ...
    def generate_all(self) -> None:
        values = self._values()
        self.curve_configs[self.active_curve_index] = dict(values)
        self._refresh_curve_tree()
        try:
            self.status.set("Generating selected I–V curve...")
            self.window.update_idletasks()
            self._generate_curve(self.active_curve_index)
            if not self.field_selected_indices:
                raise ValueError(
                    "Select one or two curves in the Field comparison panel."
                )
            self.field_outputs = {}
            for index in sorted(self.field_selected_indices):
                ...
                if mesh is None:
                    self.status.set(f"Generating Curve {index + 1} Gmsh structure...")
                    self.window.update_idletasks()
                    mesh = generate_gmsh_mesh(length, tox, self.geo_template)
                    self.mesh_cache[key] = mesh
                self.status.set(f"Generating Curve {index + 1} field map...")
                self.window.update_idletasks()
                prediction = self.field_predictor.predict(
                    mesh,
                    length,
                    tox,
                    float(config["B"]),
                ...
                self.field_outputs[index] = GeneratedFieldMap(
                    mesh,
                    prediction,
                    length,
                    tox,
            ...
            self.render_curves()
            self.render_field()
            self._mark_explanation_stale("curve")
            self._mark_explanation_stale("field")
            warnings = [
                f"Curve {index + 1}: {warning}"
                for index, config in enumerate(self.curve_configs)
                for warning in [range_warning(config)]
                if warning
            ]
            self.status.set(
                " | ".join(warnings)
                or f"{len(self.field_outputs)} field map(s) generated with a shared comparison scale"
            )
        except Exception as exc:
            messagebox.showerror(
                "Integrated model generation failed", str(exc), parent=self.window
            )
    ...
    def _generate_curve(self, index: int) -> None:
        ...
        result = (
            f"Curve {index + 1}",
            self.curve_predictor.predict("idvd", features),
            self.curve_predictor.predict("idvg", features),
        )
    ...
    def render_curves(self) -> None:
        ...
        visible_results = [
            result
            for index, result in enumerate(self.curve_results)
            if index in self.visible_curve_indices
        ]
        self.curve_figure.clear()
        if visible_results:
            render_curve_figure(
                self.curve_figure, visible_results, self.combined_curves
            )
        else:
            self.curve_figure.text(
                0.5, 0.5, "No curves selected", ha="center", va="center", fontsize=14
            )
    ...
    def _update_electrical_panel(self) -> None:
        ...
        for column, (curve_label, idvd, idvg) in enumerate(self.curve_results, start=1):
            ...
            try:
                ...
                values["ion_ioff_ratio"] = (
                    values.get("ion_ma_per_um", 0.0) / ioff if ioff != 0 else math.inf
                )
            except (ValueError, FloatingPointError):
                values = {}
            for row, (name, _label, _unit, factor, fmt) in enumerate(
                ELECTRICAL_PARAMETERS, start=1
            ):
                text = format(values[name] * factor, fmt) if name in values else "-"
                value = ttk.Label(
                    self.electrical_panel, text=text, width=11, anchor=tk.E
                )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Combined final curve and field-map model GUI"
    )
    parser.add_argument(
        "--curve-model-dir",
        type=Path,
    ...
    parser.add_argument(
        "--geo-template",
        type=Path,
        default=root / "tcad/data_extraction/base_case/gmsh_mos2d.geo",
    )
...
def main() -> int:
    args = _parse_args()
    curve_predictor = FinalCurvePredictor(args.curve_model_dir.resolve())
    field_predictor = FieldMapPredictor(args.field_model_dir.resolve())
    if args.smoke_test or args.save_dir:
        values = {name: str(getattr(args, name.lower())) for name in PARAMETERS}
        features = device_features(values)
        idvd, idvg = curve_predictor.predict("idvd", features), curve_predictor.predict(
            "idvg", features
        )
        mesh = generate_gmsh_mesh(args.l, args.t, args.geo_template.resolve())
        prediction = field_predictor.predict(
            mesh, args.l, args.t, args.b, args.sd, args.ldd
        )
        output = GeneratedFieldMap(
            mesh, prediction, args.l, args.t, args.b, args.sd, args.ldd
        )
        if args.save_dir:
            args.save_dir.mkdir(parents=True, exist_ok=True)
            curve_figure = Figure(figsize=(14, 8), dpi=120)
            FigureCanvasAgg(curve_figure)
            render_curve_figure(curve_figure, [("Curve 1", idvd, idvg)])
            curve_figure.savefig(
                args.save_dir / "integrated_curve.png", dpi=150, facecolor="white"
            )
            field_figure = Figure(figsize=(14, 8), dpi=120)
            FigureCanvasAgg(field_figure)
            render_model_field(field_figure, output, "Potential")
            field_figure.savefig(
                args.save_dir / "integrated_fieldmap.png", dpi=150, facecolor="white"
            )
        print(
            f"curve_finite={bool(np.isfinite(idvd.currents).all() and np.isfinite(idvg.currents).all())}"
        )
        print(
            f"field_nodes={len(mesh.node_xy_nm)}, field_elements={len(mesh.triangles)}, field_finite={all(np.isfinite(v).all() for v in [*prediction.node_fields.values(), *prediction.element_fields.values()])}"
        )
        return 0
    window = tk.Tk()
    IntegratedModelApp(
        window, curve_predictor, field_predictor, args.geo_template.resolve()
    )
    window.mainloop()
    return 0
