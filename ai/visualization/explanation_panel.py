...
class ExplanationPanelMixin:
    ...
    def _build_explanation_panel(self, parent: ttk.LabelFrame, kind: str) -> None:
        controls = ttk.Frame(parent); controls.pack(fill=tk.X, pady=(0, 5))
        button = ttk.Button(controls, text="Analyze", command=lambda: self._analyze(kind)); button.pack(side=tk.LEFT)
        ttk.Button(controls, text="Copy", command=lambda: self._copy_explanation(kind)).pack(side=tk.LEFT, padx=4)
        ttk.Button(controls, text="Generate Prompt", command=lambda: self._generate_prompt(kind)).pack(side=tk.LEFT, padx=(0, 4))
        provider_name = getattr(getattr(self, "explanation_service", None), "provider", None)
        status = tk.StringVar(value=f"Ready ({getattr(provider_name, 'name', 'mock')})"); ttk.Label(controls, textvariable=status).pack(side=tk.LEFT, padx=(5, 0))
    ...
    def _analyze(self, kind: str) -> None:
        try:
            if kind == "curve":
                ...
                mode = getattr(getattr(self, "explanation_mode_var", None), "get", lambda: "Analyzer-guided")()
                stages = {"Extracted-only": "extracted_only", "Evidence-assisted": "evidence_assisted"}
                task = (lambda: self.explanation_service.explain_curves_staged(results, configs, stages[mode])) if mode in stages else (lambda: self.explanation_service.explain_curves(results, configs))
            else:
                ...
                mode = getattr(getattr(self, "explanation_mode_var", None), "get", lambda: "Analyzer-guided")()
                stages = {"Extracted-only": "extracted_only", "Evidence-assisted": "evidence_assisted"}
                task = (lambda: self.explanation_service.explain_fields_staged(outputs, display, scale, range_mode, stages[mode])) if mode in stages else (lambda: self.explanation_service.explain_fields(outputs, display, scale, range_mode))
    ...
    def _generate_prompt(self, kind: str) -> None:
        try:
            ...
            if kind == "curve":
                ...
                prompt = self.explanation_service.build_curves_prompt(results, configs, mode)
            else:
                ...
                prompt = self.explanation_service.build_fields_prompt(
                    outputs, self.field_var.get(), self.scale_var.get(), self.range_var.get(), mode,
                )
        except ValueError as exc:
            messagebox.showinfo("Prompt unavailable", str(exc), parent=self.window)
            return
        dialog = tk.Toplevel(self.window)
        dialog.title(f"{kind.title()} Explanation Prompt — {mode}")
        dialog.geometry("950x720")
        actions = ttk.Frame(dialog, padding=(8, 8)); actions.pack(fill=tk.X)
        ttk.Label(actions, text=f"Mode: {mode}").pack(side=tk.LEFT)
    ...
    def _explanation_view_key(self, kind: str) -> tuple:
        ...
        if kind == "curve":
            ...
            return mode, indices, tuple(tuple(sorted(self.curve_configs[index].items())) for index in indices)
        ...
        return mode, self.field_var.get(), self.scale_var.get(), self.range_var.get(), indices, configs
