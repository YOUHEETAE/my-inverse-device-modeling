  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="mx-auto flex max-w-5xl flex-col gap-10 px-6 py-10">
        ...
        {/* ── 왜 필요한가 ──────────────────────────────────────────────
            TCAD를 흠잡지 않는다. 설계 도구로는 옳은 물건이고, 이 사이트의
            학습 데이터도 TCAD(DEVSIM)로 만들었다 — 아래 주의사항에 그렇게
            적혀 있으므로 "TCAD는 비싸다"고 잘라 말하면 같은 페이지 안에서
            말이 어긋난다. 문제는 도구의 품질이 아니라 한 번의 시도가 비싸서
            학습에 필요한 반복이 성립하지 않는다는 것이다. */}
        <section className="flex flex-col gap-3">
          <h2 className="text-sm font-bold uppercase tracking-wide text-primary">왜 필요한가</h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {[
              {
                head: "한 번 돌리는 데 오래 걸립니다",
                body: "구조 생성, Mesh 설정, 물리 모델, 수렴 조정을 거쳐야 조건 하나의 결과가 나옵니다. 정밀한 설계에는 마땅한 값이지만, 배우면서 스무 번 바꿔보기에는 한 번이 너무 비쌉니다.",
              },
              {
                head: "시작하기 전에 알아야 할 게 많습니다",
                body: "상용 도구는 라이선스가 비싸 학생이 접근하기 어렵고, 무료 도구도 메쉬와 물리 모델을 다룰 줄 알아야 첫 결과가 나옵니다. 배우려고 쓰는데 배우고 나서야 쓸 수 있습니다.",
              },
              {
                head: "그래서 바꿔볼 기회가 없습니다",
                body: "이론은 배웠는데 채널을 줄이면 소자 안에서 무슨 일이 생기는지 한 번도 못 본 채로 넘어갑니다. 이 도구는 그 사이를 메우려 합니다.",
              },
            ].map((item) => (
              <Card key={item.head}>
          ...
          <p className="text-sm leading-relaxed text-on-surface-variant">
            TCAD를 대신하려는 것이 아니라, TCAD가 맡지 않는 자리를 채우려는 것입니다. 미리 계산해 둔
            결과로 학습한 예측 모델이 답을 내놓기 때문에 조건을 바꾸고 그 자리에서 비교할 수
