...
| AI 설명 · 자유질문 · Case Study 피드백 서술 | 로컬 폴백 | 아래 참고 |
| 로그인 / 계정별 학습 기록 | 없음 | 아래 참고 |

**AI 설명과 자유질문**은 외부 LLM(Groq)을 부릅니다. API 키를 실행 파일에 넣으면
추출되므로 키 없이 배포하고, 그 경우 코드에 준비된 로컬 폴백이 답합니다. 실제
LLM 답변은 배포된 서비스에서 확인할 수 있습니다. 키를 가진 분은 실행 전
`GROQ_API_KEY` 환경변수를 설정하면 그대로 동작합니다.
