...
def _reset_to_zero_bias(dev):
    ...
    solve(type="dc", absolute_error=1.0e30, relative_error=1e-5, maximum_iterations=80)
...
_reset_to_zero_bias(device)
rampbias(device, "gate", 1.0, 0.02, RAMP_MIN_STEP, RAMP_MAX_ITER, RAMP_REL_ERROR, RAMP_ABS_ERROR, _noop)
rampbias(device, "drain", 0.0, 0.02, RAMP_MIN_STEP, RAMP_MAX_ITER, RAMP_REL_ERROR, RAMP_ABS_ERROR, _noop)
...
_reset_to_zero_bias(device)
rampbias(device, "drain", 0.05, 0.02, RAMP_MIN_STEP, RAMP_MAX_ITER, RAMP_REL_ERROR, RAMP_ABS_ERROR, _noop)
rampbias(device, "gate", -0.5, 0.02, RAMP_MIN_STEP, RAMP_MAX_ITER, RAMP_REL_ERROR, RAMP_ABS_ERROR, _noop)
...
print("=== SWEEP_START IDVG_VD1P0 ===")
_reset_to_zero_bias(device)
rampbias(device, "drain", 1.0, 0.02, RAMP_MIN_STEP, RAMP_MAX_ITER, RAMP_REL_ERROR, RAMP_ABS_ERROR, _noop)
rampbias(device, "gate", -0.5, 0.02, RAMP_MIN_STEP, RAMP_MAX_ITER, RAMP_REL_ERROR, RAMP_ABS_ERROR, _noop)
_log_ivpoint("IDVG_VD1P0", device)
rampbias(
    device,
    "gate",
    1.5,
