...
def _find_para_sweep_root(start: Path) -> Path | None:
    for parent in [start, *start.parents]:
        if parent.name == "para_sweep":
            return parent
    return None


def _load_env_config(start: Path):
    para_sweep_root = _find_para_sweep_root(start)
    if para_sweep_root:
        scripts_dir = para_sweep_root / "scripts"
...
def configure_devsim_runtime(caller_file: str) -> Path:
    ...
    para_sweep_root = _find_para_sweep_root(this_file.parent)
    if para_sweep_root:
        config_dir = para_sweep_root / "config"
