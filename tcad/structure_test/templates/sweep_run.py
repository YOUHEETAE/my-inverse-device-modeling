...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Skip structure/doping pairs whose IdVd, IdVg, and final field files already exist.",
    )
    parser.add_argument(
        "-j",
        "--jobs",
        ...
        help=(
            ...
            "(default: 1). Increase carefully if each run uses a lot of CPU or memory."
        ),
    )
...
def _extract_final_bias(stdout_text: str) -> tuple[float, float] | None:
...
def _curve_name_stem(structure_id: str, row: DopingRow) -> str:
    ...
    LDD = _format_doping_label(row.LDD_doping) if row.LDD_doping else ""
...
def _expected_output_paths(
    ...
    return (
        ...
        final_fields_dir / f"{naming}_final.dat",
    )
...
def _has_existing_outputs(
    ...
    return all(
        ...
        for path in _expected_output_paths(structure_id, row, curves_dir, final_fields_dir)
    )
...
def _skipped_result(
    ...
    idvd_csv, idvg_csv, final_dat = _expected_output_paths(
...
def _run_one(
    ...
    final_bias = _extract_final_bias(completed.stdout)
...
def _run_tasks_sequential(
    ...
    for task in tasks:
        ...
        try:
            ...
            _write_status_csv(status_csv, _ordered_results(results_by_index))
...
def main() -> None:
    ...
    for structure_dir in structures:
        ...
        for row in rows:
            ...
            if args.skip_existing and _has_existing_outputs(
    ...
    print(f"Total sweep elapsed: {sweep_elapsed:.3f}s ({_format_elapsed(sweep_elapsed)})")
