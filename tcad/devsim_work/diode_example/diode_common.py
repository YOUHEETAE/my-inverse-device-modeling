...
def _mesh_path(filename):
    return str(_EXAMPLE_DIR / filename)
...
def Create2DGmshMesh(device, region):
    ...
    create_gmsh_mesh(mesh="diode2d", file=_mesh_path("gmsh_diode2d.msh"))
...
def Create3DGmshMesh(device, region):
    # this reads in the gmsh format
    create_gmsh_mesh(mesh="diode3d", file=_mesh_path("gmsh_diode3d.msh"))
